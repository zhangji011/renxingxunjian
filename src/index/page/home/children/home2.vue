...
export default {
  ...
  mounted() {
    ...
    this.xjstatus=sessionStorage.getItem("xjstatus") || -1;
    if(this.xjstatus==0){
      this.cityName1 = "该商户位置准确且名称与营业执照一致";
    }else if(this.xjstatus==1){
      this.cityName1 = "该地址无此商户";
    }else if(this.xjstatus==2){
      this.cityName1 = "商户地址不详,找不到具体位置";
    }else if(this.xjstatus==3){
      this.cityName1 = "商户异常";
    }
    ...
    // 用$on事件来接收参数
    Bus.$on('val', (data) => {
      vm.cityName1 = data   //点完picker 改变商户状态
    })
  ...
  methods: {
    ...
    getClickCity() {// 级联选择器
          ...
          this.$weui.picker([
            {label:"商户状态",status:-1},
            {label:"该商户位置准确且名称与营业执照一致",status:0},
            {label:"该地址无此商户",status:1},
            {label:"商户地址不详,找不到具体位置",status:2},
            {label:"商户异常",status:3}
          ], {
              depth: 1,
              defaultValue: [0],
              ...
              onConfirm: function (city) {
                  that.xjstatus = city[0].status;
                  // console.log("展示商户的状态改变"+that.xjstatus)
                  sessionStorage.setItem("xjstatus",city[0].status);
    ...
    aginEnter() {
      ...
      this.initScroll();
      let that = this;
      if (this.cityName1 && this.cityName1 != "商品状态" && window.LATITUDE) {
        this.init();
      } else {
        try {
    ...
    intervalCity() {
      this.cityName1 = window.CITYNAME || "商品状态";
    },
    defaultIconF(i) {
    ...
    loadMore() {
      ...
      let morequeryUrl = "merchants/queryXjInfo";  //默认请求所有数据  只传用户id
      let morequertyParams = {  //默认请求所有，只需传一个用户id
        userId:Cookies.get("userId"),
        currentPage: this.CURRENTPAGE,
        pagNum: this.PAGNUM || 10
      }
      if(this.xjstatus!=-1){   //用户改变搜索商户状态
        morequeryUrl = "merchants/queryXjInfoByStatus";
        morequertyParams = {
          userId:Cookies.get("userId"),
          storeStatus:this.xjstatus,
          currentPage: this.CURRENTPAGE,
          pagNum: this.PAGNUM || 10
        }
      }
      axios.post(morequeryUrl, morequertyParams).then(res => {
    ...
    init(flag) {
      ...
      this.l=sessionStorage.getItem("item1");
      this.s=sessionStorage.getItem("item2");
      let queryUrl = "merchants/queryXjInfo";  //默认请求所有数据  只传用户id
      let quertyParams = {  //默认请求所有，只需传一个用户id
        userId:Cookies.get("userId"),
        currentPage: this.CURRENTPAGE,
        pagNum: this.PAGNUM || 10
      }
      if(this.xjstatus!=-1){   //用户改变搜索商户状态
        queryUrl = "merchants/queryXjInfoByStatus";
        quertyParams = {
          userId:Cookies.get("userId"),
          storeStatus:this.xjstatus,
          currentPage: this.CURRENTPAGE,
      ...
      axios.post(queryUrl, quertyParams).then(res => {
          ...
          if (res.data && res.data.length > 0) {
            ...
            this.isError = true;
            this.totalInit++;
            if (this.totalInit <= 2) {  //没有数据就再次调用axios请求
              this.init();
            } else {
              this.shopList = [];
              this.SHOWLOADING(false);
