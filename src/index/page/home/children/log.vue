    ...
    <ul class="formCom form-login form-mini">
      <li class="icon-clear">
        <label>
          ...
          <!-- <input type="number" placeholder="请输入账号"  maxlength="11" v-model.number='name'> -->
        </label>
      </li>
      ...
      <li>
        <label>
          ...
          <input type='password' placeholder="请输入密码" onkeyup="this.value=this.value.replace(/\s+/g,'')" v-model.trim='pwd'>
			...
			<div class="nullHeight"></div>
      <li>
        <canvasCode @codeHasChange='sendCode' ref="canvascode"></canvasCode>
      </li>
    </ul>
...
export default {
  ...
  methods: {
    logIn() {
      let that = this;
      //先做一些简单的判断再提交ajax
      if (/^[1][345678][0-9]{9}$/.test(this.name) === false){
        this.$dialog('帐号不正确');
      }
        ...
        this.$refs.canvascode.changeCode();  //刷新验证码
        this.$dialog('验证码不正确');
      } else{
        //先跳到首页暂代，后期ajax替换
        axios.post('user/userLogin',{
          ...
          "password": this.pwd
            ...
            if(res.code === "0") {
              ...
              localStorage.setItem("mblno",data);
              localStorage.setItem("totalBus",totalNumber);
              localStorage.setItem("userId",userId);
              // console.log(Cookies.get("mblno"));
              setTimeout(function(){
                // window.location.href = `http://localhost:8000/main.html#/my?mblno=${data}&totalBus=${totalNumber}&userId=${userId}`  //本地
                // window.location.href = "https://hbtest.letshappy.cn/rhxj/main.html#/my"  //测试地址
                 window.location.href = `https://find.cmpay.com:9102/hebao/rhxj/main.html#/my?mblno=${data}&totalBus=${totalNumber}&userId=${userId}`  //投产地址
              },20);
              // that.$router.push({
              //   path: "/my",
              //   query: {
