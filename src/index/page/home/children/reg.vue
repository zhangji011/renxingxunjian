    ...
    <ul class="formCom form-login">
      ...
      <li :class="showPwdOne?'icon-eye1':'icon-eye2'">
        <label>
          ...
          <input type="password" placeholder="请输入密码" onkeyup="this.value=this.value.replace(/\s+/g,'')" v-model='pwdOne'>
    ...
    methods: {
      goReg() {
        ...
        let checkPhone = /^[1][3578][0-9]{9}$/,
          ...
          checkPwdOne = /^(?![0-9]+$)(?![a-z]+$)(?![A-Z]+$)(?!([^(0-9a-zA-Z)]|[])+$)([^(0-9a-zA-Z)]|[]|[a-z]|[A-Z]|[0-9]){6,20}$/,
          checkSMS = /^[0-9]{4,8}$/;
        //先做一些简单的判断再提交ajax
        ...
        else if (this.pwdOne.indexOf(" ") != -1) this.$dialog('密码格式不正确');
        else {
          axios.post('user/userRegister',{
            bTelephone: this.phone,
