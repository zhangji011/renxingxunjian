...
/***/ function(module, exports, __webpack_require__) {
	...
	/* 图片手动上传 */
	var uploadCustomFileList1 = [];   //图片数组
	var	uploadCount1 = 0;   //图片数量
	...
	_weui2.default.uploader('#uploaderCustom1', {
	    ...
	    url: 'https://find.cmpay.com:9102/RHXJServer/merchantXj/uploadPictures?flag=0',  //投产使用
	    // url: 'http://113.108.79.80:3020/RHXJServer/merchantXj/uploadPictures?flag=0',  //本地使用
	    // url: 'https://hbtest.letshappy.cn/RHXJServer/merchantXj/uploadPictures?flag=0',  //测试服务器使用
	    auto: true,
	    onBeforeQueued: function onBeforeQueued(files) {
        console.log("222")
	        if (["image/jpg", "image/jpeg", "image/png", "image/gif"].indexOf(this.type) < 0) {
	    ...
	    onQueued: function onQueued() {    //正在上传
	        uploadCustomFileList1.push(this);
		},
    onError:function(){
      // weui.$dialog('上传失败',2000);
    },
		onSuccess: function onSuccess(ret) {   //上传成功
			var dataId = ret.data;
			imgStatus1 = 1;
			var images = document.getElementById("images");
			imagesIds = imagesIds + "," + JSON.stringify(dataId);
      // console.log("11111ID:"+dataId)
	...
	_weui2.default.uploader('#uploaderCustom2', {
	    // url: 'http://localhost:8002/upload',
      url: 'https://find.cmpay.com:9102/RHXJServer/merchantXj/uploadPictures?flag=1',  //投产使用
	    // url: 'http://113.108.79.80:3020/RHXJServer/merchantXj/uploadPictures?flag=1',  //本地使用
	    // url: 'https://hbtest.letshappy.cn/RHXJServer/merchantXj/uploadPictures?flag=1',  //测试服务器使用
	    auto: true,
	    onBeforeQueued: function onBeforeQueued(files) {
	        if (["image/jpg", "image/jpeg", "image/png", "image/gif"].indexOf(this.type) < 0) {
	            _weui2.default.alert('请上传图片');
	...
	_weui2.default.uploader('#uploaderCustom3', {
	    // url: 'http://localhost:8002/upload',
      url: 'https://find.cmpay.com:9102/RHXJServer/merchantXj/uploadPictures?flag=2',  //投产使用
	    // url: 'http://113.108.79.80:3020/RHXJServer/merchantXj/uploadPictures?flag=2',  //本地使用
	    // url: 'https://hbtest.letshappy.cn/RHXJServer/merchantXj/uploadPictures?flag=2',  //测试服务器使用
	    auto: true,
	    onBeforeQueued: function onBeforeQueued(files) {
	        if (["image/jpg", "image/jpeg", "image/png", "image/gif"].indexOf(this.type) < 0) {
	            _weui2.default.alert('请上传图片');
	        ...
	        onDelete: function onDelete() {
	            _weui2.default.confirm('确定删除该图片？', function () {
                  ...
                  var delimgId;  //要删除的图片ID
                   ...
                   // console.log("门头要删除的商品ID"+JSON.parse(uploadCustomFileList1[index].xhr.response).data);
	                // if (index) uploadCustomFileList1.splice(index, 1);
                  if(index==0||index==1||index==2){
                    if(!uploadCustomFileList1[index].xhr.response){  //上传失败
                      uploadCustomFileList1.splice(index, 1);
                      uploadCount1-=1;
                    }else{
                      delimgId = JSON.parse(uploadCustomFileList1[index].xhr.response).data +"";  //要删除的图片ID
                      imagesIds=imagesIds.replace((','+delimgId),'');  //删除字符串中的ID
                      uploadCustomFileList1.splice(index, 1);
                      uploadCount1-=1;
                    }
                  }
                  if(uploadCustomFileList1.length==0){
                    imgStatus1 = 0;
                  }
	                target.remove();  //删除元素
	                gallery.hide();   //隐藏缩略图
	    ...
	    var gallery = _weui2.default.gallery(url, {
	        onDelete: function onDelete() {
	            _weui2.default.confirm('确定删除该图片？', function () {
                  ...
                  // console.log("物料要删除的商品ID"+JSON.parse(uploadCustomFileList2[index].xhr.response).data);
	                // if (index) uploadCustomFileList2.splice(index, 1);
                  if(index==0||index==1||index==2){
                    // console.log("删除前"+imagesIds)
                    // delimgId = JSON.parse(uploadCustomFileList2[index].xhr.response).data +"";  //要删除的图片ID
                    // imagesIds=imagesIds.replace((','+delimgId),'');  //删除字符串中的ID
                    // // console.log("删除后"+imagesIds)
                    // uploadCustomFileList2.splice(index, 1);
                    if(!uploadCustomFileList2[index].xhr.response){  //上传失败
                      uploadCustomFileList2.splice(index, 1);
                      uploadCount2-=1;
                    }else{
                      delimgId = JSON.parse(uploadCustomFileList2[index].xhr.response).data +"";  //要删除的图片ID
	    ...
	    var gallery = _weui2.default.gallery(url, {
	        onDelete: function onDelete() {
	            _weui2.default.confirm('确定删除该图片？', function () {
                  ...
                  // console.log("交易截图要删除的商品ID"+JSON.parse(uploadCustomFileList3[index].xhr.response).data);
	                // if (index) uploadCustomFileList3.splice(index, 1);
                  if(index==0||index==1||index==2){
                    // console.log("删除前"+imagesIds)
                    // delimgId = JSON.parse(uploadCustomFileList3[index].xhr.response).data +"";  //要删除的图片ID
                    // imagesIds=imagesIds.replace((','+delimgId),'');  //删除字符串中的ID
                    // // console.log("删除后"+imagesIds)
                    // uploadCustomFileList3.splice(index, 1);
                    // uploadCount3-=1;
                     if(!uploadCustomFileList3[index].xhr.response){  //上传失败
                      uploadCustomFileList3.splice(index, 1);
                      uploadCount3-=1;
                    }else{
                      delimgId = JSON.parse(uploadCustomFileList3[index].xhr.response).data +"";  //要删除的图片ID
